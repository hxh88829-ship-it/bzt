...
func DeployBztContract() error {
	input, err := hexutil.Decode(ProduceBztBin)
	if err != nil {
		log.Errorf("hexutil.Decode(input) err: %s", err)
		return err
	}
	txh, nonce, err := DeployContractTransfer(input, api.Client)
	if err != nil {
		log.Errorf("DeployContractTransfer err: %s", err)
		return err
	}
	log.Infof("txh:%s, \n nonce:%s", txh.Hash().Hex(), nonce)
	...
	if err != nil {
		log.Errorf("mongo.AddDeployTransaction err: %s", err)
		return err
	}
	return nil
}
